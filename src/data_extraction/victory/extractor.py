...
from src.data_extraction.data_extraction_config import VICTORY_RAW_DATA_DIR
from src.data_extraction.local_files import (
    list_local_price_full_files,
    require_local_files,
)
from src.data_extraction.victory.download import download_price_full_files
...
class VictoryExtractor(ChainExtractor):
    def extract_price_full(self) -> list[Path]:
        ...
        local_files = list_local_price_full_files(VICTORY_RAW_DATA_DIR)
        print(
            FOUND_LOCAL_FILES_MESSAGE.format(count=len(local_files)),
            flush=True,
        )
        return require_local_files(local_files, VICTORY_RAW_DATA_DIR)
