...
class VictoryExtractor(ChainExtractor):
    def extract_price_full(self) -> list[Path]:
        ...
        local_files = list_local_price_full_files(VICTORY_PRICE_FULL_RAW_DATA_DIR)
        print(
            FOUND_LOCAL_FILES_MESSAGE.format(count=len(local_files)),
            flush=True,
        )
        return require_local_files(local_files, VICTORY_PRICE_FULL_RAW_DATA_DIR)
    ...
    def extract_stores(self) -> list[Path]:
        ...
        print(SKIPPING_DOWNLOAD_MESSAGE, flush=True)
        local_files = list_local_stores_files(VICTORY_STORES_RAW_DATA_DIR)
        print(
            FOUND_LOCAL_FILES_MESSAGE.format(count=len(local_files)),
            flush=True,
        )
    ...
    def extract_promo_full(self) -> list[Path]:
        ...
        print(SKIPPING_DOWNLOAD_MESSAGE, flush=True)
        local_files = list_local_promo_full_files(VICTORY_PROMO_FULL_RAW_DATA_DIR)
        print(
            FOUND_LOCAL_FILES_MESSAGE.format(count=len(local_files)),
            flush=True,
        )
