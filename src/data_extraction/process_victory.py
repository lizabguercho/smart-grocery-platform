...
def get_local_price_files() -> list[Path]:
    VICTORY_RAW_DATA_DIR.mkdir(parents=True, exist_ok=True)
    return sorted(VICTORY_RAW_DATA_DIR.glob("*PriceFull*"))
...
def main(
    *,
    ...
    if download:
        print("1. Getting Victory PriceFull files...", flush=True)
        ...
        if not downloaded_files:
            raise FileNotFoundError(
                f"No PriceFull files found in {VICTORY_RAW_DATA_DIR}. "
    ...
    utils.save_to_csv(products, VICTORY_CHAIN_NAME)
